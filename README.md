...
백엔드가 콘솔 창 없이 트레이 아이콘 하나로 뜹니다(우클릭 메뉴로 로그 확인/종료).
`start.cmd`를 다시 실행하면 기존 프로세스를 정리하고 새로 띄웁니다(중복 안 됨).
트레이 아이콘이 없어졌거나 어떤 이유로든 종료가 안 될 때는 `stop.cmd`를 더블클릭하면
llama-server·백엔드·트레이를 전부 강제 종료합니다. 아래는 수동으로 하나씩 띄우는
방법(디버깅 등에 필요할 때).
