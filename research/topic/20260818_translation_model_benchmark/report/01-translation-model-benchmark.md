...
- **오차범위(95% CI)**: 페어드 부트스트랩(120개 세그먼트를 복원추출로 재표본,
  500회 반복). 코퍼스 단위 chrF++는 매 재표본마다 처음부터 재계산. 사람 채점
  평균 점수는 세그먼트별 점수의 평균을 동일하게 부트스트랩. 비율(S1율)은 Wilson
  score interval.
...
| 모델 | 전체 chrF++ (95% CI) | normal | hard | grammar 호환 |
|---|---|---|---|---|
| Qwen2.5-7B-Instruct (베이스라인) | 24.64 ± 2.87 | 25.46 ± 3.49 | 22.99 ± 5.18 | 정상 |
| **Qwen3-14B** | **31.44** (CI 산출 불가 — 아래 참고) | 31.45 | 31.32 | **비호환**(꺼야 동작) |
| **Gemma-3-12b-it** | 28.69 ± 3.49 | 30.04 ± 4.15 | 26.05 ± 6.50 | 정상 |
| EXAONE-3.5-7.8B-Instruct | 22.37 ± 2.48 | 21.69 ± 2.61 | 23.89 ± 4.75 | 정상(베이스라인보다 낮음) |

> Qwen3-14B는 리포트 작성 시점(2026-08-22 소급 정리) 기준 `data/`에 저장된
> `bench_qwen3-14b.jsonl`이 grammar 켠 붕괴 버전(chrF++ ~10대)뿐이라, 표에 적힌
> 31.44(grammar 끈 정상 버전)를 낸 원본 세그먼트별 출력이 남아있지 않아 CI를 다시
> 계산할 수 없었다. 다른 세 모델은 원본 세그먼트 데이터가 남아있어 정확히 재계산.
...
동일 `hyp_ja`(STT 가설) 조건에서 재계산한 chrF++ (± 95% CI):

| 그룹 | chrF++ (hyp_ja→hyp_ko, 실제 파이프라인) | chrF++ (ja_ref→hyp_ko_from_ref, STT 오류 배제) |
|---|---|---|
| normal (94) | 23.20 ± 4.07 | 30.04 ± 4.15 |
| hard (26) | 22.30 ± 5.05 | 26.05 ± 6.50 |
| 전체 (120) | **22.86 ± 3.39** | **28.69 ± 3.49** |

사람(LLM) 채점 4개 항목 (± 95% CI, n=120):

| 지표 | 베이스라인 (Qwen2.5-7B) | Gemma-3-12b-it | 변화 |
|---|---|---|---|
| 의미충실도 | 2.72 ± 0.28 | 3.02 ± 0.30 | **+0.30** |
| 자연스러움 | 2.85 ± 0.22 | 3.57 ± 0.30 | **+0.72** |
| 존댓말일치 | 2.74 ± 0.17 | 3.22 ± 0.27 | **+0.48** |
| S1율 | 50.8% ± 8.8%p | 43.3% ± 8.7%p | **-7.5%p** |
| chrF++ (전체, hyp_ko) | 17.33 ± 2.55 | 22.86 ± 3.39 | +5.53 |
| chrF++ (전체, ja_ref 직접번역) | 24.25 ± 2.83 | 28.69 ± 3.49 | +4.44 |
| S1 원인 중 STT 전파 비율 | 69% | 82.7% | +13.7%p |

두 모델의 의미충실도/자연스러움/존댓말일치 CI는 서로 겹치지 않아(예: 자연스러움
2.85±0.22 [상한 3.07] vs 3.57±0.30 [하한 3.27]) 이 세 항목의 개선은 표본 변동으로
설명되지 않는 수준. chrF++(hyp_ko)와 S1율은 CI가 좁게 겹치지만 방향은 일관됨.
S1 원인 중 STT 전파 비율은 원본 세그먼트별 라벨(`s1_cause`)에 대한 Wilson CI를
별도로 계산하지 않았다 — 참고용 보조 지표로만 표기.
...
- `chrf_by_model.png` — 4개 모델 chrF++(정답 전사 직접번역), 전체/normal/hard
- `gemma_vs_baseline_chrf.png` — 베이스라인 vs Gemma, 실제 파이프라인 vs STT 오류 배제 조건
- `human_scores.png` — 사람(LLM) 채점 3개 항목(의미충실도/자연스러움/존댓말일치)
- `s1_rate.png` — S1율(핵심의미 파손 비율)
