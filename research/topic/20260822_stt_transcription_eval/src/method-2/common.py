"""Shared dataset loading + text normalization for method-1.

Standalone from the live-translator app on purpose (user instruction,
2026-08-22): this evaluates the faster-whisper STT model directly, not the
app's WebSocket/VAD/session pipeline. No `backend.*` imports.
"""
...
if __name__ == "__main__":
    segs = load_dataset()
    print(f"loaded {len(segs)} segments across {len(CATEGORIES)} categories")
    by_cat: dict[str, int] = {}
    for s in segs:
