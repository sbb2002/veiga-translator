"""granite-speech-4.1-2b (transformers, CPU) on the pilot subset. Same output
schema as transcribe_turbo.py so score_quantitative.py works unmodified.
...
Usage: python transcribe_granite.py [--pilot-n 5] [--limit N]
"""
...
def main() -> None:
    ...
    model = AutoModelForSpeechSeq2Seq.from_pretrained(MODEL_ID, device_map=DEVICE, torch_dtype=torch.float32)
    model.eval()
