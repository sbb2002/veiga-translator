...
### 정성 평가 (50세그먼트 수동 채점, fair 추가 2026-08-28)

`report/02`의 50세그먼트 표본(seed=7, 카테고리당 10개)에 `reazonspeech-nemo-v2_fair`를
같은 두 축으로 채점(Claude, 2026-08-28). `src/qualitative_eval.py agg`,
95% CI = 부트스트랩 2000회. 점수는 `out/qualitative_scores.json`, 대조표는
`out/qualitative_sample.txt`.

| 방법 | 자연스러움 (95% CI) | 의미 충실도 (95% CI) | 유창한 환각률 |
|---|---|---|---|
| large-v3-turbo | 4.14 (3.82~4.44) | 3.44 (3.00~3.86) | 8/50 (16%) |
| Qwen3-ASR-1.7B | 3.74 (3.46~4.00) | 3.54 (3.12~3.94) | 3/50 (6%) |
| ReazonSpeech bare | 3.56 (3.22~3.88) | 3.22 (2.78~3.66) | 2/50 (4%) |
| **ReazonSpeech fair (CPU)** | **4.00 (3.60~4.36)** | **3.64 (3.22~4.04)** | 4/50 (8%) |

**페어드 차이 (A − B), 95% CI:**

| 비교 | 자연스러움 | 의미 충실도 |
|---|---|---|
| fair − bare | +0.44 [+0.18, +0.70] | +0.42 [+0.12, +0.74] |
| fair − turbo | −0.14 [−0.56, +0.24] (0 포함) | **+0.20 [+0.02, +0.36]** |
| fair − Qwen3-ASR-1.7B | +0.26 [−0.08, +0.58] (0 포함) | +0.10 [−0.14, +0.34] (0 포함) |

**카테고리별 의미 충실도** (fair): 게임 2.10 / 여행 2.50 / 음식,요리 4.10 /
**일상,소통 4.60**(bare 3.50 → 회복) / 패션,뷰티 4.90. 자연스러움 분해:
자연 전체 4.00, 충실≥3 4.56, 충실≤2 2.57, r(자연,충실) 0.82.

**해석:**
- **정성에서도 래퍼가 두 축 모두 유의미하게 개선**(fair − bare CI가 0 제외).
  `일상,소통` 의미 충실도 3.50 → 4.60 — 정량의 문두 회복이 정성으로도 확인.
- **정성에서는 fair가 turbo/Qwen과 대등하거나 낫다**: 의미 충실도 `fair − turbo`
  +0.20 [+0.02, +0.36]로 **fair가 turbo보다 유의미하게 높고**, Qwen과는 동급.
  자연스러움은 셋 다 동급. → **정량(4개 지표 전부 fair가 유의미하게 뒤짐)과
  방향이 다르다.**
- 이유: CER/chrF++는 게임 반복 루프(`ピンポン…`×20 = 대량 삽입)와 표기 차이
  (`米田`→`コメダ`, `御茶ノ水`→`お茶の水`)를 크게 벌하지만, 사람이 "의미가
  전달됐나"로 보면 표기 오류는 fid 4, 문두 회복은 fid를 크게 올린다. 게임
  카테고리(10/50)의 재앙이 코퍼스 CER은 지배하고 정성 평균에는 희석된다.
- **유창한 환각률 8%** — bare 4%보다 오르지만(래퍼가 짧고 매끄러운 오역을 냄:
  `来る必要がある` 등) turbo 16%의 절반. r(자연,충실) 0.82로 "매끄러우면 대체로
  맞음"은 유지 — fail-loud 성격 대체로 보존.
