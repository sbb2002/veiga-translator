...
/ 20260826_stt_model_survey: 500 resamples, corpus-level metrics recomputed
from scratch each resample, not a mean of per-segment values) for the 4
methods' out/<method>/quant_results.csv over the full 150-pair set, plus
bar-chart PNGs with error bars into fig/.

All 4 methods share the same 150 segments (same seg_id order), so per-segment
